...
export default function AboutSection() {
  return (
    // Cambiamos 'animate' por 'whileInView' para que la animación ocurra al hacer scroll o abrir el acordeón
    <motion.div
      variants={containerVariants}
      initial="hidden"
      whileInView="visible"
      viewport={{ once: true, margin: "-50px" }}
      className="pt-8 pb-16 border-t border-foreground/10 flex flex-col gap-6"
    >

      <div className="mb-4">
        <h2 className="text-4xl md:text-6xl font-black tracking-tighter text-foreground uppercase">
          Sobre <span className="text-transparent bg-clip-text bg-gradient-to-br from-[#E61C8C] to-[#FF5C33]">Mí.</span>
        </h2>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 auto-rows-[auto]">
        {/* Bio */}
        <motion.div variants={itemVariants} className="md:col-span-2 bg-foreground/[0.02] border border-foreground/10 rounded-[2rem] p-8 md:p-12 shadow-sm relative overflow-hidden group hover:border-[#A3249E]/30 transition-colors duration-500">
          ...
          <div className="relative z-10">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-foreground/5 border border-foreground/10 mb-8">
              <span className="w-2 h-2 rounded-full bg-[#E61C8C] animate-pulse" />
            ...
            <p className="text-2xl md:text-4xl font-medium text-foreground/90 leading-tight mb-6 tracking-tight">
              Hola, soy Nicolás. Tengo 22 años y soy desarrollador especializado en construir <strong className="font-black">software multiplataforma de alto rendimiento</strong>.
            </p>
        ...
        {/* Foto */}
        <motion.div variants={itemVariants} className="md:col-span-1 min-h-[350px] md:min-h-full bg-foreground/5 rounded-[2rem] overflow-hidden border border-foreground/10 hover:border-[#E61C8C]/30 relative group transition-colors duration-500">
          ...
          <div className="absolute bottom-8 left-8 right-8 flex justify-between items-end">
            <div className="flex flex-col">
              ...
              <span className="text-[9px] uppercase tracking-widest text-[#FF5C33] font-bold mt-1">Tizayuca, MX</span>
            </div>
            <div className="w-12 h-12 rounded-full border border-foreground/20 flex items-center justify-center backdrop-blur-sm group-hover:rotate-180 group-hover:border-[#E61C8C]/50 transition-all duration-700">
              <span className="text-[#FF5C33] text-xs shadow-[0_0_10px_#FF5C33]">●</span>
            </div>
          </div>
        </motion.div>

        {/* Stack */}
        <motion.div variants={itemVariants} className="col-span-1 bg-gradient-to-br from-[#E61C8C]/5 to-[#A3249E]/5 border border-foreground/10 hover:border-[#A3249E]/30 rounded-[2rem] p-8 flex flex-col justify-between transition-colors duration-500">
          <div className="flex flex-wrap gap-3 mb-8">
            {/* Animación continua flotante para los íconos (desfasada por delay) */}

            {/* 1. Flutter */}
            <motion.div animate={{ y: [0, -4, 0] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut", delay: 0 }} className="w-10 h-10 rounded-xl bg-background border border-foreground/10 flex items-center justify-center hover:scale-110 transition-transform duration-300 shadow-sm" title="Flutter">
            ...
            {/* 2. React */}
            <motion.div animate={{ y: [0, -4, 0] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut", delay: 0.5 }} className="w-10 h-10 rounded-xl bg-background border border-foreground/10 flex items-center justify-center hover:scale-110 transition-transform duration-300 shadow-sm" title="React">
            ...
            {/* 3. Node.js */}
            <motion.div animate={{ y: [0, -4, 0] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut", delay: 1 }} className="w-10 h-10 rounded-xl bg-background border border-foreground/10 flex items-center justify-center hover:scale-110 transition-transform duration-300 shadow-sm" title="Node.js">
            ...
            {/* 4. Python */}
            <motion.div animate={{ y: [0, -4, 0] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut", delay: 1.5 }} className="w-10 h-10 rounded-xl bg-background border border-foreground/10 flex items-center justify-center hover:scale-110 transition-transform duration-300 shadow-sm" title="Python">
            ...
            {/* 5. PHP / Laravel */}
            <motion.div animate={{ y: [0, -4, 0] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut", delay: 2 }} className="w-10 h-10 rounded-xl bg-background border border-foreground/10 flex items-center justify-center hover:scale-110 transition-transform duration-300 shadow-sm" title="PHP / Laravel">
            ...
            {/* 6. Docker */}
            <motion.div animate={{ y: [0, -4, 0] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut", delay: 2.5 }} className="w-10 h-10 rounded-xl bg-background border border-foreground/10 flex items-center justify-center hover:scale-110 transition-transform duration-300 shadow-sm" title="Docker">
        ...
        {/* Stats */}
        <motion.div variants={itemVariants} className="col-span-1 bg-foreground/[0.02] border border-foreground/10 rounded-[2rem] p-8 flex flex-col justify-center gap-4 hover:border-[#A3249E]/30 transition-colors duration-500">
        ...
        <motion.div variants={itemVariants} className="col-span-1 group h-full">
          <a href="/Nicolas_Hernandez_CV.pdf" target="_blank" rel="noopener noreferrer" className="w-full h-full bg-[#000000] border border-white/10 hover:border-transparent text-white rounded-[2rem] p-8 flex flex-col items-center justify-center hover:scale-[0.97] hover:shadow-[0_0_30px_-5px_#E61C8C] transition-all duration-500 relative overflow-hidden">
            ...
            <div className="absolute inset-0 bg-gradient-to-r from-[#E61C8C] via-[#A3249E] to-[#FF5C33] opacity-0 group-hover:opacity-10 transition-opacity duration-500" />

            <div className="w-16 h-16 rounded-full bg-white/5 border border-white/10 flex items-center justify-center mb-4 group-hover:bg-[#E61C8C]/20 group-hover:border-[#E61C8C]/50 transition-colors duration-500">
              <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="group-hover:text-[#FF5C33] transition-colors duration-500">
