...
export default function Home() {
  ...
  // --- ESTADO PARA CONTROLAR EL INDICADOR DE SCROLL ---
      ...
      {!loading && (
        <>
          <section className="relative w-full min-h-screen flex flex-col items-center justify-center overflow-hidden bg-transparent px-6 pt-32 pb-20 transition-colors duration-500">
            ...
            {/* CUADRÍCULA DE FONDO FUTURISTA */}
            <div className="absolute inset-0 bg-grid z-0 pointer-events-none opacity-60" />
            ...
            {/* FONDO DINÁMICO (Paleta Sintaxis Lab) */}
            ...
            <motion.div initial="hidden" animate="visible" transition={{ staggerChildren: 0.15 }} className="relative z-10 max-w-7xl w-full">
              <div className="space-y-8 max-w-5xl">
                ...
                <motion.h1 variants={fadeInUp} className="text-5xl sm:text-6xl md:text-8xl lg:text-[9rem] font-black tracking-tighter leading-[0.85] uppercase text-foreground">
                ...
                <motion.div variants={fadeInUp} className="flex flex-wrap gap-5 pt-4">
                  ...
                  {/* Botón Ver Proyectos (Estilo Terminal / Sintaxis Lab) */}
                  <button
            ...
            <AnimatePresence>
              {activeSection === 'none' && showScrollIndicator && (
                <motion.div 
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 0.4, y: 0 }}
                  exit={{ opacity: 0, y: 10 }}
                  transition={{ duration: 0.5 }}
                  className="absolute bottom-10 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 pointer-events-none hidden md:flex z-10"
                >
                  <span className="text-[10px] tracking-[0.25em] uppercase text-foreground/60 font-black">Deslizar</span>
                  <div className="w-6 h-10 border border-foreground/30 rounded-full flex justify-center p-1.5 bg-background/20 backdrop-blur-sm">
                    <motion.div 
                      animate={{ y: [0, 12, 0] }}
                      transition={{ repeat: Infinity, duration: 1.6, ease: "easeInOut" }}
                      className="w-1.5 h-2 bg-gradient-to-b from-[#E61C8C] to-[#FF5C33] rounded-full shadow-[0_0_8px_#E61C8C]"
                    />
                  </div>
                </motion.div>
              )}
